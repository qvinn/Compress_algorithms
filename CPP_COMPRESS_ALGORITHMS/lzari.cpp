...
// Инициализация дерева
 /* For i = 0 to N - 1, rson[i] и lson[i] будут правым и левым потомками узла i. Эти узлы не нуждаются в инициализации. dad[i] является родителем узла i. Они инициализируются как NIL (= N), означающее 'не использовано'
  * For i = 0 to 255, rson[N + i + 1] - корни деревьев для строк, которые начнаются с символа i. Они инициализируются как NIL. Всего 256 деревьев */
...
//---------------------------------------------------------
// Вставка узла
/* Вставляет строку text_buf[r..r+F-1] длины F в одно из деревьев (text_buf[r]'th tree) и возвращает позицию и длину через глобальные переменные match_position и match_length. Если match_length = F,
 * то старый узел замещается новым. r играет двойную роль - как узел дерева и позиция в буфере */
...
void LZAri::StartModel(void) {
    ...
    position_cum[N] = 0;
    for(i = N; i >= 1; i--) {
        position_cum[i - 1] = position_cum[i] + 10000 / (i + 200);               /* функция распределения подобрана эмпирически */
    }
}
...
int LZAri::BinarySearchSym(unsigned int x) {           /* 1 if x >= sym_cum[1], N_CHAR if sym_cum[N_CHAR] > x, i such that sym_cum[i - 1] > x >= sym_cum[i] otherwise */
    int i;
    int j;
    int k;
    i = 1;
...
int LZAri::BinarySearchPos(unsigned int x) {               /* 0 if x >= position_cum[1], N - 1 if position_cum[N] > x, i such that position_cum[i] > x >= position_cum[i + 1] otherwise */
    int i;
    int j;
    int k;
    i = 1;
